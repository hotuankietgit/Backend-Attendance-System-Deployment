...
class AttendanceDetailController {
    constructor() {
        ...
        this.getTotalStatsByClassIDForAdmin = (req, res) => __awaiter(this, void 0, void 0, function* () {
            try {
                const classID = req.params.id;
                let { data: classData, error: errorClass } = yield ClassService_1.default.getClassesWithStudentsCourseTeacher(classID);
                if (errorClass) {
                ...
                let { data: studentDetails, error: err } = yield StudentClassService_1.default.getStudentsAttendanceDetailsByClassID(classID);
                if (err) {
                ...
                let { total, pass, ban, warning, data: result } = AttendanceDetailDTO_1.default.transformStudentsAttendanceDetails(studentDetails, offset);
                let { data, error } = yield AttendanceDetailService_1.default.getStatsBasedOnClassID(classID);
                if (error) {
                    return res.status(500).json({ message: error });
                }
                if (data.length == 0) {
                ...
                let { progressPresent, progressLate, progressAbsent, groupBarCharts } = AttendanceDetailDTO_1.default.getAttendanceDetailsStats(data);
        ...
        this.getTotalStatsByClassIDForTeacher = (req, res) => __awaiter(this, void 0, void 0, function* () {
            try {
                const classID = req.params.id;
                const teacherID = req.payload.userID;
                let { data: classData, error: errorClass } = yield ClassService_1.default.getClassesWithStudentsCourseTeacher(classID);
