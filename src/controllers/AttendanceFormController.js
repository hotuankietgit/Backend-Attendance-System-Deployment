...
class AttendanceFormController {
    constructor() {
        //oke
        this.activateAttendanceForm = (req, res) => __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                let formInCheck = ClassesDTO_1.default.getFormByFormID(classes.attendanceForm, attendanceForm);
                if (formInCheck == null)
                    return res.status(503).json({ message: "Attendance Form does not exist. Please try again!" });
                let isFormInUse = formInCheck.inUsed;
                //Create entities before inserting into database
                const attendanceFormEntity = AttendanceFormService_1.default.createFormEntity(attendanceForm, classes, startTime, endTime, dateOpen, type, location, latitude, longitude, radius);
                const attendanceDetailEntities = AttendanceDetailService_1.default.createDefaultAttendanceDetailEntitiesForStudents(classes.studentClass, attendanceForm);
                //Make transactions to insert into database
                const { data: form, error } = yield AttendanceFormService_1.default.createFormTransaction(attendanceFormEntity, attendanceDetailEntities, isFormInUse);
                if (error) {
